...
my_diff_test_helper <- function(x, 
                             fullModelFormulaStr, 
    ...
    if (is.null(disp_func) == FALSE){
      ...
      if (is.null(disp_guess) == FALSE && disp_guess > 0 && is.na(disp_guess) == FALSE  ) {
        ...
        if (expressionFamily@vfamily == "negbinomial")
          ...
          expressionFamily <- VGAM::negbinomial.size(size=1/disp_guess)
      }
    }
  ...
  test_res <- tryCatch({
    ...
    my_compareModels(list(full_model_fit), list(reduced_model_fit))
  }, 
  #warning = function(w) { FM_fit },
  error = function(e) { 
    if(verbose)
      print (e);
      data.frame(status = "FAIL", family=expressionFamily@vfamily, pval=1.0, qval=1.0, full_model_loglikelihood=NA, reduced_model_loglikelihood=NA)
...
my_compareModels <- function(full_models, reduced_models){
  ...
  # Julien: Only change in functions -> add loglikelihood of both models in result
  test_res$full_model_loglikelihood = unlist(lapply(full_models, function(l) l@criterion$loglikelihood))
  test_res$reduced_model_loglikelihood = unlist(lapply(reduced_models, function(l) l@criterion$loglikelihood))

  test_res
}


my_smartEsApply <- function(X, MARGIN, FUN, convert_to_dense, ...) {
  parent <- environment(FUN)
  if (is.null(parent))
    parent <- emptyenv()
  e1 <- new.env(parent=parent)
  Biobase::multiassign(names(Biobase::pData(X)), Biobase::pData(X), envir=e1)
  environment(FUN) <- e1
  
  # if (isSparseMatrix(exprs(X))){
  #   res <- sparseApply(exprs(X), MARGIN, FUN, convert_to_dense, ...)
  # }else{
    res <- apply(Biobase::exprs(X), MARGIN, FUN, ...)
  # }
...
my_differentialGeneTest <- function(cds, 
  ...
  # if (cores > 1){
  #   diff_test_res<-mcesApply(cds, 1, my_diff_test_helper, 
  ...
  #   diff_test_res
  # }else{
    diff_test_res<-my_smartEsApply(cds,1,my_diff_test_helper, 
  ...
  diff_test_res <- merge(diff_test_res, Biobase::fData(cds), by="row.names")
